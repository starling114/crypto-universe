...
class Ads:
    ...
    async def open_url(self, url, timeout=30, sleep_time=None):
        logger.debug(f"Profile: {self.label} | Opening url: {url}")
    ...
    async def click_element(self, selector, timeout=5, sleep_time=None, source=None):
        logger.debug(f"Profile: {self.label} | Clicking element: {selector}")
    ...
    async def input_text(self, selector, text, timeout=5, delay=0.1, sleep_time=None):
        logger.debug(f"Profile: {self.label} | Inputting text: {selector} -> {text}")
        delay = delay * 1000
    ...
    async def find_element(self, selector, timeout=5, sleep_time=None, source=None):
        logger.debug(f"Profile: {self.label} | Finding element: {selector}")
        timeout = timeout * 1000
        source = source or self.page

        element = None
        try:
            element = await source.wait_for_selector(selector, timeout=timeout)
            if sleep_time:
                sleep(sleep_time)
        except TimeoutError:
            pass
    ...
    async def while_present(self, selector, timeout=5):
        logger.debug(f"Profile: {self.label} | While present: {selector}")
        timeout = timeout * 1000
        try:
    ...
    async def until_present(self, selector, timeout=5):
        logger.debug(f"Profile: {self.label} | Until present: {selector}")

        return await self.find_element(selector=selector, timeout=timeout) is not None
